...
class DefaultController extends HController
{
  ...
  public function actionPaying(){
    ...
    $this->writeLog('Log_Data',$privatekey);
    $checksum = md5($config_data[0]['AIRPAY_MERCHANT_ID']."~".$data['USER_ID']."~192.168.1.184/partnerpay/web/bbps/default/bill_data_response~192.168.1.184/partnerpay/web/bbps/default/account_register_response");
    $apidata=[
  ...
  public function actionPaymentresponse(){
    ...
    if($_POST['TRANSACTIONPAYMENTSTATUS']=='SUCCESS'){
      ...
      $chk = new Checksum();
      $privatekey =$chk->encrypt($config_data[0]['AIRPAY_USERNAME'].":|:".$config_data[0]['AIRPAY_PASSWORD'], $config_data[0]['AIRPAY_SECRET_KEY']);
      $checksum = md5($confid_data[0]['AIRPAY_MERCHANT_ID'].'~'.$_POST['AMOUNT'].'~'.Yii::$app->request->post('APTRANSACTIONID').'~192.168.1.184/partnerpay/web/bbps/default/paymentstatus~');
      $apidata=[
        'private_key'=>$privatekey,
        'mercid'=>$confid_data[0]['AIRPAY_MERCHANT_ID'],
  ...
  public function actionPaymentstatus(){
    ...
    if($posted_checksum == $checksum){
      $this->writeLog("Log_Data","Working");
      echo $posted_checksum.'_'.$checksum;
    } else {
      $this->writeLog("Log_Data","Failed");
      echo $posted_checksum.'_'.$checksum;
    }
    if($data2->STATUS == 'Y'){
      $status = "success";
